...
// The array is sorted from very light loads to heavy loads to test GPU boundaries
struct NLMWorkload {
    int search_radius;
    int patch_radius;
};
...
// Smoothing parameter applied within the NLM algorithm calculations
...
// Standard GPU Kernel utilizing global memory accesses for parallel execution
__global__ void nlm_kernel(const unsigned char* input, unsigned char* output,
                            int width, int height,
                            int search_radius, int patch_radius, float h)
{
    // Map thread indices to global 2D image coordinates
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    
    // Discard any threads that fall outside the actual image boundaries
    ...
    for (int dy = -search_radius; dy <= search_radius; ++dy) {
        for (int dx = -search_radius; dx <= search_radius; ++dx) {
            ...
            // Calculate similarity between patches using direct global memory reads
            for (int py = -patch_radius; py <= patch_radius; ++py) {
                for (int px = -patch_radius; px <= patch_radius; ++px) {
                    // Apply clamping logic to safely handle image borders without out-of-bounds errors
