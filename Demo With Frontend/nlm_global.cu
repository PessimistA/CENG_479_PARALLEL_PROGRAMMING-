...
// Basic GPU Kernel relying purely on Global Memory
// This approach is straightforward but generally slower due to repeated memory fetching latency
__global__ void nlm_kernel(const unsigned char* input, unsigned char* output,
                            int width, int height,
                            int search_radius, int patch_radius, float h)
{
    // Determine the global image coordinates assigned to this specific thread
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    
    // Discard threads that fall outside the image dimensions
    ...
    // Directly access global memory for the search window
    for (int dy = -search_radius; dy <= search_radius; ++dy) {
        for (int dx = -search_radius; dx <= search_radius; ++dx) {
            int sx = x + dx;
            int sy = y + dy;
            ...
            // Extract patch similarities, forcing a global memory access for every single pixel checked
            for (int py = -patch_radius; py <= patch_radius; ++py) {
                for (int px = -patch_radius; px <= patch_radius; ++px) {
                    // Apply clamping logic to handle edges without reading outside the allocated array
                    int px1 = min(max(x  + px, 0), width  - 1);
                    int py1 = min(max(y  + py, 0), height - 1);
                    int px2 = min(max(sx + px, 0), width  - 1);
                    int py2 = min(max(sy + py, 0), height - 1);
                    float d = (float)input[py1 * width + px1]
                            - (float)input[py2 * width + px2];
                    patch_dist += d * d;
                }
                        ...
                        int repeats)
{
    ...
    for (int b = 0; b < N_BLOCK_SIZES; ++b) {
        ...
        for (int r = 0; r < repeats; ++r) {
            ...
            // Launch the standard global memory kernel without dynamic shared memory parameters
            nlm_kernel<<<blocks, threads>>>(d_input, d_output, W, H,
                                            search_radius, patch_radius, H_PARAM);
...
int main(int argc, char** argv)
{
    ...
    std::string json_path = out_dir + "/results_global.json";
