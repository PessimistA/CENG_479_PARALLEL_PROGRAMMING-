...
// The list prioritizes lighter loads but includes a heavy load (11, 3) to test GPU limits
struct NLMWorkload {
    int search_radius;
    int patch_radius;
};
...
// Predefined thread block sizes to sweep and find the most efficient execution configuration
static const int BLOCK_SIZES[][2] = {{8,8},{16,16},{32,32}};
static const int N_BLOCK_SIZES    = 3;

// Structure to store execution times for different block dimensions
struct BlockResult {
    int bw;
    int bh;
...
// Structure to measure the quality of the filtered image against the CPU baseline
struct Metrics {
    double mse;
...
// This runs sequentially and serves as the ground truth for quality and speedup calculations
             ...
             int search_radius, int patch_radius, float h)
{
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            ...
            // Search window loop to find similar patches
            for (int dy = -search_radius; dy <= search_radius; ++dy) {
                for (int dx = -search_radius; dx <= search_radius; ++dx) {
                    ...
                    // Calculate the squared Euclidean distance between the reference patch and the current patch
                    ...
                    // Assign a weight based on patch similarity
...
__global__ void nlm_kernel(const unsigned char* input, unsigned char* output,
                            ...
                            int search_radius, int patch_radius, float h)
{
    // Dynamically allocated shared memory to store the image tile and its surrounding halo
    ...
    // Calculate the size of the halo region required for the search and patch operations
    int R = search_radius + patch_radius;
    int tile_w = blockDim.x + 2 * R;
    int tile_h = blockDim.y + 2 * R;

    // Determine the top-left starting coordinate of the block including the halo area
    ...
    // Synchronize all threads in the block to ensure the shared memory tile is fully populated
    __syncthreads();

    // Threads mapped outside the actual image boundaries can exit after participating in the memory load
    ...
    // Perform the NLM calculation entirely using the fast shared memory array
    for (int dy = -search_radius; dy <= search_radius; ++dy) {
        for (int dx = -search_radius; dx <= search_radius; ++dx) {
...
// Calculates the Mean Squared Error (MSE) to verify GPU computational accuracy against the CPU baseline
...
// Appends the current workload benchmark results into a consolidated JSON file for the Python GUI
                               ...
                               const Metrics& m)
{
    ...
    // Build the JSON object structure manually
    ...
    // Insert the new record before the closing bracket of the JSON array
                        ...
                        int repeats)
{
    ...
    // Allocate memory on the GPU device and transfer the input image
    ...
    // Sweep through the predefined block sizes to identify the optimal configuration
    ...
    for (int b = 0; b < N_BLOCK_SIZES; ++b) {
        ...
        for (int r = 0; r < repeats; ++r) {
            ...
            // Pass the calculated shared memory size as the 3rd launch configuration parameter
        ...
        // Keep a copy of the result generated by the fastest block configuration
    ...
    // Release the allocated device memory to prevent leaks
    cudaFree(d_input);
    cudaFree(d_output);
    ...
    // Calculate Amdahl's Law theoretical parallelization ratio
...
int main(int argc, char** argv)
{
    ...
    std::string json_path = out_dir + "/results_shared.json";
